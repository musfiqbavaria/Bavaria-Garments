...
class Command(BaseCommand):
    ...
    def handle(self,*a,**o):
        ...
        FinishingAutoReport.objects.update_or_create(report_date=d,slot=s,defaults={"summary":payload,"outstanding_alerts":Alert.objects.filter(actioned=False).count(),"pending_actions":ActionItem.objects.exclude(status="COMPLETED").count(),"escalated_items":Alert.objects.filter(actioned=False,level="RED").count()})
